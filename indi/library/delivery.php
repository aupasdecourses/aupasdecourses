...
trait name {
	private function canCreditmemo(Mage_Sales_Model_Order $order)
	{
		...
		if ($order->canUnhold()
			...
			|| $order->isCanceled()
			|| $order->getState() === self::STATE_CLOSED
			|| abs($order->getStore()->roundPrice($order->getTotalPaid()) - $order->getTotalRefunded()) < .0001
			|| $order->getActionFlag(self::ACTION_FLAG_EDIT) === false) {
			return false;
		}
		return true;
	}
	...
	public function processcreditAction($orderId, $total, $data)
	{

		//transformation
		//clean
		//invoice
		//creditmemo by merchant

/* transformation		//get total to refund per commercant
		$totalrefund = array();
		foreach ($data as $row) {
