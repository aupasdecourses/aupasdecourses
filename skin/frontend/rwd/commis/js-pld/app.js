...
$j(document).ready(function () {
    ...
    //Tentative to put hover function on skip links in header
    
    // skipLinks.hover(function () {
    ...
    //     if(target.data("hovered")!='True'){skipContents.removeClass('skip-active')}
    // });

    // skipContents.hover(function(){
    //     $j(this).data("hovered",'True');
    // },
    //     function(e){
    //     $j(this).removeClass('skip-active');
    //     $j(this).data('hovered')
    ...
    $j('input[name^="cart"]').keypress(function(event){
        if(event.keyCode == 13){
            event.preventDefault();
            $j(this).siblings('button').click();
        }
    });
    
    if ($j('body.checkout-cart-index').length) {
        $j('textarea[id="comment-cart"]').focus(function () {
            $j('#update_all_cart:button').addClass('btn-need-update');
        });
    }
