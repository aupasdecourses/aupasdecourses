...
function all_orders($var='mwddate',$commercantId='all'){
    try{
      ...
      if($var=='mwddate'){
        ...
        $orders->getSelect()->join( array('order_attribute'=> 'amasty_amorderattr_order_attribute'), 'order_attribute.order_id = main_table.entity_id', array("produit_equivalent" => 'order_attribute.produit_equivalent', "codeporte1" => 'order_attribute.codeporte1'));
      ...
      if($commercantId<>'all'){
        $orders->getSelect()->join(array('order_item' => Mage::getSingleton('core/resource')->getTableName('sales/order_item')),
          'order_item.order_id = main_table.entity_id')->where('order_item.commercant='.strval($commercantId))->group('order_item.order_id');
      }
...
function datetime_filter($render='date',$var="mwddate"){
  try{
    ...
    if($render=="date"){
      ...
      if(!in_array($current_day,$temp)){$temp[]=$current_day;};
      rsort($temp);
    }

    if($render=="time"){sort($temp);}
...
function data_facturation_products($debut,$fin){
  ...
  $fin = date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $fin)));
 ...
 //In case of modified order, find the id of the order made by the client
    ...
    // }else{
      $ordered_items=$order->getAllVisibleItems();
      $order_comments=getRelevantComments($order);
    // }

    //Invoiced Items
    if ($order->hasInvoices()) {$invoices=$order->getInvoiceCollection();};
    ...
    foreach($list_commercant as $id => $com){
          ...
          if ($order->hasInvoices()){
            ...
            if($sum_items!=0 || ($sum_items_invoice != 0 && $order->hasInvoices())) {
                //Parent ID
                $date=date("d/m/Y", Mage::getModel("core/date")->timestamp($order->getCreatedAt()));
                if($parentid==NULL){$parentid=$order->getIncrementId();}
                ...
                array_push($data,[
                  ...
                  // 'sum_items_invoice'=>$sum_items_invoice,
                  // 'sum_items_invoice_HT'=>$sum_items_invoice_HT,
                  // 'sum_items_invoice_TVA'=>$sum_items_invoice_TVA,
                  'sum_items_credit'=>$sum_items_credit,
                  'sum_items_credit_HT'=>$sum_items_credit_HT,
                  'sum_items_credit_TVA'=>$sum_items_credit_TVA,
                  'remboursements'=>$order_comments,
