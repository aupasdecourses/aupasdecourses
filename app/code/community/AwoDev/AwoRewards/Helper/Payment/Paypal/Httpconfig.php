...
class AwoDev_AwoRewards_Helper_Payment_Paypal_Httpconfig extends AwoDev_AwoRewards_Helper_Payment_Paypal {
	 ...
	 */
	public function init($url, $method=self::HTTP_POST) {
		$this->url = $url;
		$this->method = $method;
	...
	public function setHttpProxy($proxy) {
		...
		if($urlParts == false || !array_key_exists("host", $urlParts))
			throw new Exception("Invalid proxy configuration ".$proxy);
	...
	public function handle($request) {
		...
		switch($request->bindingType) {
			case 'NV':
					$this->addHeader('X-PAYPAL-SECURITY-USERID', $credential->userName);
					$this->addHeader('X-PAYPAL-SECURITY-PASSWORD', $credential->password);
					$this->addHeader('X-PAYPAL-SECURITY-SIGNATURE', $credential->signature);
				break;
			/*case 'SOAP':
		...
		$this->addHeader('X-PAYPAL-REQUEST-DATA-FORMAT', $request->bindingType);
		$this->addHeader('X-PAYPAL-RESPONSE-DATA-FORMAT', $request->bindingType);
		$this->addHeader('X-PAYPAL-DEVICE-IPADDRESS', Mage::helper('awodev_aworewards/payment_paypal_utils')->getLocalIPAddress());
		$this->addHeader('X-PAYPAL-REQUEST-SOURCE', Mage::helper('awodev_aworewards/payment_paypal_service')->getRequestSource());
		if(!empty($credential->applicationId)) {
			$this->addHeader('X-PAYPAL-APPLICATION-ID', $credential->applicationId);
		}
