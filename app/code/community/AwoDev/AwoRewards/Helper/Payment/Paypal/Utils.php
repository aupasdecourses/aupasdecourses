...
class AwoDev_AwoRewards_Helper_Payment_Paypal_Utils extends AwoDev_AwoRewards_Helper_Payment_Paypal {
	 ...
	 * @return array
	 */
	public function nvpToMap($nvpString)
	{
		$ret = array();
	 ...
	 * @return bool
	 */
	public function array_match_key($map, $key)
	{
	...
	public function xmlToArray($xmlInput)
	{
		...
		$array = array();
		$ret = $this->convertXmlObjToArr($getChild, $array);
		return $ret;
	}
	 ...
	 * @return string
	 */
	public function escapeInvalidXmlCharsRegex($textContent)
	{
	 ...
	 * @return array
	 */
	public function filterKeyPrefix(array $map, $keyPrefix)
	{
		$filtered = array();
...
if(!isset(self::$propertiesRefl[$class])) exit('blowme');
	 ...
	 * @return string
	 */
	public function isAttributeProperty($class, $propertyName) {
		if (($annotations = $this->propertyAnnotations($class, $property))) {
	 ...
	 * @return string
	 */
	public function isPropertyArray($class, $propertyName) {
		if (($annotations = $this->propertyAnnotations($class, $propertyName))) {
	 ...
	 * @return string
	 */
	public function propertyType($class, $propertyName)
	{
		if (($annotations = $this->propertyAnnotations($class, $propertyName)) && isset($annotations['var'])) {
	 ...
	 * @return array
	 */
	public function objectProperties($object)
	{
		$props = array();
		foreach (get_object_vars($object) as $property => $default) {
			$annotations = $this->propertyAnnotations($object, $property);
	 ...
	 * @return array
	 */
	public function lowerKeys(array $array)
	{
		$ret = array();
