<?php
class Pmainguet_Commercants_Block_Adminhtml_Editcustomer extends Mage_Adminhtml_Block_Customer_Edit_Tab_Account
{
	 ...
	 public function initForm()
    {
        ...
        #Javascript to hide/show commercant_id - Pierre Mainguet
        $field = $form->getElement('group_id');
        $field->setData('onchange','change(this.value)');
		$field->setAfterElementHtml("<script>

		        function change(selectItem){

		            var e = document.getElementById('_accountgroup_id');
		            var arr = new Array();
		            ...
		            console.log(arr);
		            var values = arr.indexOf('7');
		            if (values != -1) { 
		                document.getElementById('_accountcommercant_id').style.display = '';
		            } else {
		                document.getElementById('_accountcommercant_id').style.display = 'none';   
		            } 
		        }
		        </script>");

		$other = $form->getElement('commercant_id');
