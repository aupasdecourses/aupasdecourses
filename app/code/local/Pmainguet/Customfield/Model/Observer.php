<?php
class Pmainguet_Customfield_Model_Observer{
    /**
    ...
    public function saveQuoteBefore($evt){
        ...
        if(isset($post['custom']['doorcode'])){
            $var = $post['custom']['doorcode'];
            $quote->setDoorcode($var);
        }
    }
    /**
    ...
    public function saveQuoteAfter($evt){
        $quote = $evt->getQuote();
        if($quote->getDoorcode()){
            $var = $quote->getDoorcode();
            if(!empty($var)){
                $model = Mage::getModel('custom/custom_quote');
                $model->deteleByQuote($quote->getId(),'doorcode');
                $model->setQuoteId($quote->getId());
                $model->setKey('doorcode');
                $model->setValue($var);
                $model->save();
            }
        }
    ...
    public function saveOrderAfter($evt){
        ...
        if($quote->getDoorcode()){
            ...
            if(!empty($var)){
                ...
                $model->deleteByOrder($order->getId(),'doorcode');
                $model->setOrderId($order->getId());
                $model->setKey('doorcode');
                $model->setValue($var);
                $order->setDoorcode($var);
                $model->save();
            }
        }
    }
