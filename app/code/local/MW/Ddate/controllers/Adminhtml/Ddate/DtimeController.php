...
class MW_Ddate_Adminhtml_Ddate_DtimeController extends Mage_Adminhtml_Controller_Action
{
    protected function _isAllowed()
    {
        return Mage::getSingleton('admin/session')->isAllowed('ddate/dtime');
    }
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->getParam('dtime_id') > 0) {
            try {
                $ddate = Mage::getModel('ddate/ddate')->getCollection()->addFieldToFilter('dtime', $this->getRequest()->getParam('dtime_id'));
                if (count($ddate->getData()) != 0) {
                    $dtime = Mage::getModel('ddate/dtime')->load($this->getRequest()->getParam('dtime_id'));
                    $dtime->delete();
                    Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Item was successfully deleted'));
                } else {
                    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('You can not delete this item'));
                    $this->_redirect('*/*/edit', array('dtime_id' => $this->getRequest()->getParam('dtime_id')));
                }
                $this->_redirect('*/*/');
            } catch (Exception $e) {
    ...
    {
        $dtimeIds = $this->getRequest()->getParam('dtime_id');
        $deleted = 0;
        if (!is_array($dtimeIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($dtimeIds as $dtimeId) {
                    $dtime = Mage::getModel('ddate/dtime')->load($dtimeId);
                    $dtime->delete();
                    $deleted++;
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__('Total of %d record(s) were successfully deleted', $deleted)
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
