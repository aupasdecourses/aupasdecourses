...
class Apdc_Delivery_Model_Orders_Shop extends Mage_Sales_Model_Order_Item
{
    ...
    public function getShops()
    {
        ...
        $sql = 'SELECT main_table.*, apdc_commercant_contact.lastname AS m_lastname, apdc_commercant_contact.firstname AS m_firstname, apdc_commercant_contact.email AS m_email, apdc_commercant_contact.phone AS m_phone, apdc_commercant_contact.lastname AS e1_lastname, apdc_commercant_contact.firstname AS e1_firstname, apdc_commercant_contact.email AS e1_email, apdc_commercant_contact.phone AS e1_phone, apdc_commercant_contact.lastname AS e2_lastname, apdc_commercant_contact.firstname AS e2_firstname, apdc_commercant_contact.email AS e2_email, apdc_commercant_contact.phone AS e2_phone FROM apdc_shop AS main_table LEFT JOIN apdc_commercant_contact ON main_table.id_contact_manager=apdc_commercant_contact.id_contact LEFT JOIN apdc_commercant_contact AS apdc_commercant_contact_2 ON main_table.id_contact_employee=apdc_commercant_contact.id_contact LEFT JOIN apdc_commercant_contact AS apdc_commercant_contact_3 ON main_table.id_contact_employee_bis=apdc_commercant_contact.id_contact';
    ...
    public function getShopsOrdersAction($dfrom = null, $dto = null)
    {
        if (!isset($dfrom)) {
            $dfrom = date('Y-m-d');
        }
        ...
        foreach ($items as $item) {
            if ($this->checkItem($item->getCommercant())) {
                ...
                $item = $item->toArray(
                    array('commercant', 'item_id', 'qty_ordered', 'row_total_incl_tax', 'produit_fragile')
                );
